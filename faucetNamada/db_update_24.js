...
async function updateStatus() {
  ...
  try {
    ...
    const [rows] = await connection.execute('SELECT * FROM faucetNama');
    ...
    for (let row of rows) {
      ...
      if (duration.asHours() < 24) {
        ...
        await connection.execute('UPDATE faucetNama SET status = ? WHERE id = ?', ['blocked', row.id]);
      } else {
        // Обновляем статус на 'working'
        await connection.execute('UPDATE faucetNama SET status = ? WHERE id = ?', ['working', row.id]);
      }
    }
